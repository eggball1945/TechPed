<div class="max-w-7xl mx-auto px-4 sm:px-6 py-16 sm:py-24">
    ...
    <div class="flex items-end justify-between mb-12 animate-fade-in-up">
        <div class="space-y-4">
            ...
            <h2 class="text-4xl sm:text-5xl font-black text-gray-900 tracking-tighter">
                Telusuri <span class="text-violet-700">Produk</span>
            </h2>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
        @php
            $categories = ['PC', 'Mini PC', 'Laptop', 'Gaming', 'Hardware'];
            $currentCategory = request('category');
        @endphp

        @foreach ($categories as $cat)
            @php
                $routeParams = ['category' => $cat];
                if (request('search')) {
                    $routeParams['search'] = request('search');
                }
            @endphp

            <a href="{{ route('user.products', $routeParams) }}"
                class="group flex flex-col items-center justify-center p-8 rounded-[2rem] border-2 transition-all duration-500 cursor-pointer animate-fade-in-up
                       {{ $currentCategory === $cat
                           ? 'bg-violet-700 border-violet-700 text-white shadow-2xl shadow-violet-200 -translate-y-2'
                           : 'bg-white border-gray-50 text-gray-900 shadow-xl shadow-gray-200/20 hover:bg-violet-700 hover:border-violet-700 hover:text-white hover:shadow-2xl hover:shadow-violet-200 hover:-translate-y-2' }}"
                style="animation-delay: {{ $loop->index * 100 }}ms">
                
                {{-- SVG Icon Container --}}
                <div class="mb-4 transform group-hover:scale-110 transition-transform duration-500 {{ $currentCategory === $cat ? 'text-white' : 'text-gray-900 group-hover:text-white' }}">
                    @if ($cat === 'PC')
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="w-12 h-12 md:w-16 md:h-16">
                    ...
                    @elseif ($cat === 'Mini PC')
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" class="w-12 h-12 md:w-16 md:h-16">
                    ...
                    @elseif ($cat === 'Laptop')
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 80 80" class="w-12 h-12 md:w-16 md:h-16">
                    ...
                    @elseif ($cat === 'Gaming')
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 15 15" class="w-12 h-12 md:w-16 md:h-16">
                    ...
                    @else {{-- Hardware --}}
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-12 h-12 md:w-16 md:h-16">
                ...
                <span class="text-xs md:text-sm font-black transition-colors duration-300 uppercase tracking-widest text-center">
                    {{ $cat }}
                </span>
            </a>
        @endforeach
