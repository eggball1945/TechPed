...
<div class="p-6">
    ...
    <div class="w-full bg-white rounded-md border border-gray-300 p-4 shadow-sm">
        <div class="overflow-x-auto">
            <table class="w-full border-collapse">
                <thead>
                    <tr class="bg-gray-50/50 border-b border-slate-200">
                        <th class="text-left text-[11px] font-bold py-3 px-4 text-gray-400 uppercase tracking-wider">Nama</th>
                        <th class="text-left text-[11px] font-bold py-3 px-4 text-gray-400 uppercase tracking-wider">Email</th>
                        <th class="text-left text-[11px] font-bold py-3 px-4 text-gray-400 uppercase tracking-wider">Telepon</th>
                        <th class="text-left text-[11px] font-bold py-3 px-4 text-gray-400 uppercase tracking-wider">Tanggal</th>
                        <th class="text-left text-[11px] font-bold py-3 px-4 text-gray-400 uppercase tracking-wider">Status</th>
                        <th class="text-center text-[11px] font-bold py-3 px-4 text-gray-400 uppercase tracking-wider">Aksi</th>
                    </tr>
                </thead>

                <tbody>
                    @forelse ($messages as $msg)
                        <tr class="border-b border-slate-100 hover:bg-slate-50 transition-all duration-200 {{ !$msg->is_read ? 'bg-violet-50/20 font-semibold' : '' }}">
                            <td class="py-4 px-4 text-[11px] text-gray-700 font-semibold">
                                {{ $msg->name }}
                            </td>
                            <td class="py-4 px-4 text-[11px] text-gray-700 font-medium">{{ $msg->email }}</td>
                            <td class="py-4 px-4 text-[11px] text-gray-700">{{ $msg->phone }}</td>
                            ...
                            <td class="py-3 px-2 text-center">
                                <div class="flex justify-center gap-2">
                                    <a href="{{ route('petugas.contact.show', $msg->id) }}" class="w-[24px] h-[24px] flex items-center justify-center rounded cursor-pointer hover:bg-slate-200 transition-colors duration-150">
                                    ...
                                    <form action="{{ route('petugas.contact.destroy', $msg->id) }}" method="POST" onsubmit="confirmAction(event, 'Hapus pesan ini?')">
                                        @csrf
                                        @method('DELETE')
