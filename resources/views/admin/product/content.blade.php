<div class="w-[1070px] bg-white rounded-md border border-gray-300 p-4">
    <form method="GET" action="{{ route('admin.product.index') }}" class="w-full flex items-center gap-3 mb-4">
        <div class="flex-1 relative">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari Produk" class="w-full h-6 bg-slate-50 pl-7 pr-2 text-[10px] rounded border border-slate-200 focus:border-violet-700 focus:outline-none transition"/>
        </div>

        <div class="flex gap-2 items-center">
            <div class="relative">
                <button id="sortButton" type="button" class="h-6 px-2 border border-gray-100 rounded text-[10px] flex items-center justify-between w-32 bg-white">
                    ...
                    <tr class="border-b border-slate-100 hover:bg-slate-50 transition">
                        ...
                        <td class="py-3 px-2 text-center">
                            <div class="flex justify-center gap-2">
                                ...
                                <form action="{{ route('admin.product.destroy', $product->id) }}" method="POST" onsubmit="confirmAction(event, 'Hapus produk ini?')">
                                    @csrf
                                    @method('DELETE')
